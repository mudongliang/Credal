...
int pc_legal_access(elf_core_info* core_info, elf_binary_info *bin_info, struct elf_prstatus thread){
	...
	fprintf(stdout, "Evedence: === The PC Value Is %x\n", (unsigned)address);
	char line[64];
	x86_format_insn(&inst, line, 64, intel_syntax);	
	fprintf(stdout, "Evidence: === The PC Pointing To Instruction Is %s. It Is Accessing Illegal Address\n", line);
#endif
